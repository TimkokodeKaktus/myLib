...
void Itemstore::addBook(std::string a_title, std::string a_authorName, std::string a_authorSurname, bool a_status)
{
    id++;
    bookList.push_back(new Book(id, a_title, a_authorName, a_authorSurname, a_status));
}
...
void Itemstore::showAllInStock()
{
	for(int i=0;i<bookList.size();i++) {
		if(getStatus(i) == 1){
			std::cout <<"ID:" <<i+1<< " Book :";
